...
export default function Home() {
  ...
  const [specialScope, setSpecialScope] = useState<'year' | 'month'>('month');
  ...
  const specialRecords = useMemo(() => filtered.filter((item) => /chúa nhật|sunday|lễ trọng|lễ kính chúa/i.test(`${item.rank} ${item.title}`) || /tết|thanksgiving/i.test(item.title)).filter((item) => {
    if (specialScope === 'year') return true;
    const date = parseDateKey(item.date);
    return date.getFullYear() === calendarMonth.getFullYear() && date.getMonth() === calendarMonth.getMonth();
  }), [filtered, specialScope, calendarMonth]);
  ...
  const weekDays = useMemo(() => {
    ...
    return Array.from({ length: 7 }, (_, index) => {
      ...
      return { date, key: dateKey(date), records: records.filter((record) => record.date === dateKey(date)) };
    });
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto grid max-w-[1440px] gap-7 px-5 py-7 lg:grid-cols-[340px_minmax(0,1fr)] lg:px-10">
        <aside>
          ...
          <div className="mb-3 flex items-center justify-between"><h2 className="font-heading text-lg font-semibold text-[color:var(--navy)]">Lịch trong tuần</h2><span className="text-xs text-[color:var(--ink-muted)]">Thứ Hai – Chúa Nhật</span></div>
          <div className="mb-6 space-y-2">{weekDays.map((day) => <button key={`week-${day.key}`} onClick={() => setSelectedDate(day.key)} className={`w-full rounded-xl border p-3 text-left transition ${day.key === selectedDate ? 'border-[color:var(--gold)]/45 bg-white shadow-[0_8px_28px_rgba(25,46,62,.07)]' : 'border-transparent bg-transparent hover:bg-white'}`}><div className="flex items-start gap-3"><span className={`w-16 shrink-0 text-xs ${day.key === selectedDate ? 'font-extrabold text-[color:var(--navy)]' : 'font-bold text-[color:var(--gold)]'}`}>{day.key}</span><div className="min-w-0 flex-1">{day.records.length ? day.records.slice(0, 2).map((item, index) => <span key={`${day.key}-${index}`} className={`block line-clamp-1 font-heading text-sm ${index === 0 ? 'font-semibold text-[color:var(--navy)]' : 'mt-1 text-xs font-medium text-[color:var(--ink-muted)]'}`}>{item.title}</span>) : <span className="block text-sm text-[color:var(--ink-muted)]">Ngày thường</span>}</div></div></button>)}</div>
          <div className="mb-3 flex items-center justify-between"><h2 className="font-heading text-lg font-semibold text-[color:var(--navy)]">Ngày lễ sắp tới</h2><div className="flex rounded-lg bg-[color:var(--cream)] p-0.5 text-[11px] font-semibold"><button onClick={() => setSpecialScope('year')} className={`rounded-md px-2.5 py-1 ${specialScope === 'year' ? 'bg-white text-[color:var(--navy)] shadow-sm' : 'text-[color:var(--ink-muted)]'}`}>Cả năm</button><button onClick={() => setSpecialScope('month')} className={`rounded-md px-2.5 py-1 ${specialScope === 'month' ? 'bg-white text-[color:var(--navy)] shadow-sm' : 'text-[color:var(--ink-muted)]'}`}>Tháng hiện tại</button></div></div><div className="space-y-2">{specialRecords.map((item) => <button key={`special-${item.date}`} onClick={() => setSelectedDate(item.date)} className={`w-full rounded-xl border p-3 text-left transition ${item.date === selected.date ? 'border-[color:var(--gold)]/45 bg-white shadow-[0_8px_28px_rgba(25,46,62,.07)]' : 'border-transparent bg-transparent hover:bg-white'}`}><div className="flex items-center gap-3"><span className="w-16 shrink-0 text-xs font-bold text-[color:var(--gold)]">{item.date}</span><span className="line-clamp-2 font-heading text-sm font-semibold text-[color:var(--navy)]">{item.title}</span></div></button>)}{!specialRecords.length && <p className="rounded-xl bg-white p-4 text-sm text-[color:var(--ink-muted)]">Chưa có lễ đặc biệt trong phạm vi này.</p>}</div>
        </aside>
